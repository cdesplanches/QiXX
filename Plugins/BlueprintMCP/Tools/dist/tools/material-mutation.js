...
export function registerMaterialMutationTools(server) {
    ...
    server.tool("set_material_property", "Set a top-level property on a Material (domain, blendMode, twoSided, shadingModel).", {
    ...
    server.tool("add_material_expression", "Add a new expression node to a Material's graph.", {
        material: z.string().describe("Material name or package path (e.g. 'M_MyMaterial')"),
        ...
        if (err)
            return { content: [{ type: "text", text: err }] };
        const body = { material, expressionClass, posX, posY };
        if (dryRun)
            body.dryRun = true;
    ...
    server.tool("delete_material_expression", "Delete an expression node from a Material's graph.", {
        material: z.string().describe("Material name or package path (e.g. 'M_MyMaterial')"),
    ...
    server.tool("connect_material_pins", "Connect two pins in a Material's graph.", {
        material: z.string().describe("Material name or package path (e.g. 'M_MyMaterial')"),
        ...
        if (err)
            return { content: [{ type: "text", text: err }] };
        const body = { material, sourceNodeId, sourcePinName, targetNodeId, targetPinName };
        if (dryRun)
            body.dryRun = true;
    ...
    server.tool("disconnect_material_pin", "Disconnect all links from a specific pin in a Material's graph.", {
        material: z.string().describe("Material name or package path (e.g. 'M_MyMaterial')"),
        ...
        if (err)
            return { content: [{ type: "text", text: err }] };
        const body = { material, nodeId, pinName };
        if (dryRun)
            body.dryRun = true;
    ...
    server.tool("set_expression_value", "Set the value of a material expression (constants, parameter defaults, custom code, etc.).", {
        material: z.string().describe("Material name or package path (e.g. 'M_MyMaterial')"),
        ...
        if (err)
            return { content: [{ type: "text", text: err }] };
        const body = { material, nodeId, value };
        if (parameterName)
    ...
    server.tool("move_material_expression", "Move a material expression node to a new position in the graph editor.", {
        material: z.string().describe("Material name or package path (e.g. 'M_MyMaterial')"),
        ...
        if (err)
            return { content: [{ type: "text", text: err }] };
        const body = { material, nodeId, posX, posY };
        if (dryRun)
            body.dryRun = true;
