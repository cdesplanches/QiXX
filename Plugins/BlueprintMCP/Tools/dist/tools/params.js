...
export function registerParamTools(server) {
    server.tool("change_function_parameter_type", `Change a function or custom event parameter's type to a new C++ struct type. Works with both Blueprint functions (K2Node_FunctionEntry) and custom events (K2Node_CustomEvent). Reconstructs the node to update output pins. Call refresh_all_nodes afterwards to propagate changes to downstream Break nodes. ${TYPE_NAME_DOCS} Pass dryRun=true to preview changes without saving.`, {
